...
from smipoly.smip import polg_optim as polg
...
if __name__ == "__main__":
    ...
    logging.info(
        f"[+] Loaded the Classified Dataframe to memory: Shape {cls_monomer_df.shape}"
    )
    # implementing multi threaded
    # res_mt = polg.biplym_mt(
    #     cls_monomer_df, targ=["exc_ole"], dsp_rsl=True, max_workers=MAX_WORKERS
    # )
    # implemeting single threaded
    res_mt = polg.biplym(cls_monomer_df, targ=["exc_ole"], dsp_rsl=True)

    # output for theshold implementing
    # res_mt.to_csv(f"polymer_output_w_rxn_{thresh_str}.csv")
    # logging.info("[+] Completed The polymerizations")
    # logging.info(f"[+] saved the result at >>> polymer_output_w_rxn_{thresh_str}.csv")

    # output for direct implementation
    res_mt.to_csv(f"test_5_output.csv")
    logging.info("[+] Completed The polymerizations")
    logging.info(f"[+] saved the result at >>> test_5_output.csv")
