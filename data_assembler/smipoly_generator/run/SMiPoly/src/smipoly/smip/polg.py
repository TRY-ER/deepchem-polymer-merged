            ...
            if len(temp1) != 0:
                if targ_mon2 != "none":  # case for co-polymerization
                    ...
                    if len(temp2) != 0:
                        ...
                        print(
                            "[++] Starting processing for combinations : {len(combs)} "
                        )
                        DF_temp["polym"] = DF_temp.apply(
                            lambda x: [genmol(x["mon1"]), genmol(x["mon2"])],
                            axis=1,  # using rdkit to convert to mol object from SMILES
                        ).apply(
                            bipolymA,  # this mapping basically takes the reaction and reactants to form a SMARTS based reaction and return a sequence based on the target type
                            targ_rxn=targ_rxn,
                            monL=monL,
                            Ps_rxnL=Ps_rxnL,
                            P_class=P_class,
                    ...
                    DF_temp["polym"] = DF_temp.apply(
                        ...
                        homopolymA,  # this mapping basically takes the reaction and reactants to form a SMARTS based reaction and return a sequence based on the target type
                        mons=mons,
                        excls=excls,
                        targ_mon1=targ_mon1,
                        Ps_rxnL=Ps_rxnL,
