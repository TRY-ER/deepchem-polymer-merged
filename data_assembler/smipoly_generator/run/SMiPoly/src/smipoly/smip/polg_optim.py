...
from .funclib import bipolymA, coord_polym, genmol, homopolymA
...
with open(os.path.join(db_file, "ps_rxn.pkl"), "rb") as f:
    Ps_rxnL = pickle.load(
        f
    )  # contains dictionary of { "index": <RDKit.ChemReaction object> ... }
with open(os.path.join(db_file, "ps_class.json"), "r") as f:
    Ps_classL = json.load(
        f
    )  # contains json of { "polymer_class / polymerization type ": int(num_monomer_classes) }
with open(os.path.join(db_file, "ps_gen.pkl"), "rb") as f:
    Ps_GenL = pickle.load(
        f
    )  # contains dictionary of { "polymer_class / polymerization type ": tuple("mon_type_1", "mon_type_2", "reaction_SMARTS") }

monLg = {
    int(k): v for k, v in monL.items()
}  # this holds a dictionary in the form { "monomer_index" : ["reaction_smarts", ...]}
exclLg = {
    int(k): v for k, v in exclL.items()
}  # this holds a dictionary in the form { "monomer_index" : ["eclusion_reaction_smarts", ...]}
mon_dic_inv = {
    int(k): v for k, v in mon_dic_inv.items()
}  # this holds a dictionary in the form { "monomer_index" : "monomer_type "}
...
def biplym(df, targ=None, dsp_rsl=None):
    ...
    # FOR FUTURE WORKS!! temporary reduced the dictionary for first 3 indicies list on 04/21/2004
    monL = {
        k: v for k, v in monLg.items() if k in mon_vals[0] + mon_vals[1] + mon_vals[2]
    }
    exclL = {
        k: v for k, v in exclLg.items() if k in mon_vals[0] + mon_vals[1] + mon_vals[2]
    }
    ...
    # generate polymer
    for P_class in targL:  # iteration through target classes
        for P_set in Ps_GenL[
            str(P_class)
        ]:  # iteration through the reaction specific tuples (mon_type_1, mon_type_2, reaction_SMARTS)
            targ_mon1 = ""
            targ_mon2 = ""
            targ_mon1 = P_set[0]
            targ_mon2 = P_set[1]
            ...
            Ps_rxnL_key = [
                ...
                if AllChem.ReactionToSmarts(v)
                == AllChem.ReactionToSmarts(
                    P_set[2]
                )  # cross_checking the SMARTS from different sources
            ]

            DF10 = DF[DF[targ_mon1]]  # getting dataframe of specific monomer class
            temp1 = list(
                DF10["smip_cand_mons"]
            )  # creating list of candidates from the specific class
            if len(temp1) != 0:
                if targ_mon2 != "none":  # case for co-polymerization
                    if (
                        targ_mon1 not in relevant_mon_classes
                        and targ_mon2 not in relevant_mon_classes
                    ):
                        print(
                            f"[--] Monomer {targ_mon1} or {targ_mon2} not in relevant monomer classes"
                        )
                        continue

                    print(f"[++] starting processing for {targ_mon1} + {targ_mon2}")
                    DF20 = DF[DF[targ_mon2]]
                    temp2 = list(DF20["smip_cand_mons"])
                    del DF10, DF20
                    if len(temp2) != 0:
                        combs = [
                            [m1, m2] for m1 in temp1 for m2 in temp2
                        ]  # basically zipping the combinations
                        temp11 = []
                        temp21 = []
                        for comb in combs:
                            if (
                                comb[0] != comb[1]
                            ):  # making sure the combinations avoid duplicated unit in one pair
                        ...
                        DF_temp["Ps_rxn_smarts"] = AllChem.ReactionToSmarts(
                            targ_rxn
                        )  # Add SMARTS data to the output
                        print(
                            f"[++] Starting processing for combinations : {len(combs)} "
                        )
                        DF_temp["polym"] = DF_temp.apply(
                            lambda x: [genmol(x["mon1"]), genmol(x["mon2"])],
                            axis=1,  # using rdkit to convert to mol object from SMILES
                        )

                        # Convert to list to use with multi-threading
                        mols_list = DF_temp["polym"].tolist()
                        polym_results = [None] * len(mols_list)
                        ...
                        with concurrent.futures.ThreadPoolExecutor(
                            ...
                            futures = [
                                ...
                                for i, mols in enumerate(mols_list)
                            ]
                            try:
                                for idx, future in concurrent.futures.as_completed(
                                    (f for _, f in futures), timeout=MAX_WAIT
                                ):
                                    try:
                                        print("[++] starting process")
                                        value = future.result(timeout=TIMEOUT_LIMIT)
                                        print(f"[++] recieving values > {value}")
                                        polym_results[idx] = value
                                        futures.remove(future)
                                    except concurrent.futures.TimeoutError:
                                        print("Timeout processing molecule")
                                        future.cancel()
                                        futures.remove(future)
                                ...
                                print(f"Timeout processing future task")
                            except Exception as e:
                                print(f"Error processing future task : {e}")
                                pass
                        # Assign results back to the DataFrame
                        print("[++] This part is getting executed ")
                        DF_temp["polym"] = polym_results
                        DF_temp["polymer_class"] = str(P_class)
                        DF_Pgen = pd.concat(
                            [DF_Pgen, DF_temp], ignore_index=True, copy=False
                        )  # concat the dataframes
                else:  # case for Homopolymerization
                    if targ_mon1 not in relevant_mon_classes:
                        print(
                            f"[--] Monomer {targ_mon1} not in relevant monomer classes"
                        )
                        continue
                    print(f"[++] starting processing for {targ_mon1}")
                    ...
                    with concurrent.futures.ThreadPoolExecutor(
                        ...
                        for i, future in enumerate(futures):
                            try:
                                val = future.result(timeout=TIMEOUT_LIMIT)
                                polym_results[i] = val
                            except Exception as e:
                                print(f"Error processing molecules {i}: {e}")
                                pass
...
def ole_copolym(df, targ=None, ncomp=None, dsp_rsl=None, drop_dupl=None):
    ...
    if not isinstance(targ, list):
        ...
        return
    for x in targ:
        ole_cls_trag_valid = [mon_dic_inv[k] for k in mon_vals[3]] + list(
            Ps_GenL["rec:coord"]
        )
        if x not in ole_cls_trag_valid:
            print(
                "oops! no such olefin class!\n",
    ...
    ole_targL = [
        e
        for e in list(zip(ole_clsL, cand_monsL))
        if True
        in [
    ...
    if dsp_rsl:
        print(
            "Number of generated (co)polymer, ",
            ncomp,
            " component(s) system : ",
            format(len(DF_gendP), ","),
        )
...
def _process_polymer_generation_chunk(args):
    ...
    # temp1 is the chunk, temp2 is the full list (or empty for homopolymer)
    mols1 = [genmol(m) for m in temp1]

    if targ_mon2 != "none" and temp2:
        mols2 = [genmol(m) for m in temp2]
        zipped1 = list(zip(temp1, mols1))
        zipped2 = list(zip(temp2, mols2))
    ...
    DF_temp = pd.DataFrame({"mon1": data_mon1, "mon2": data_mon2, "polym": data_polym})
...
def biplym_mt(df, targ=None, dsp_rsl=None, max_workers=None):
    """
    Multithreaded version of biplym with data-parallel chunking.
    Generates polymers based on the input DataFrame and
    specified target polymer classes using multiple processes.
    """
    if targ == None:
        targ = [
            "all",
    ...
    # Aim for multiple chunks per core to balance load
    # But simple logic: Total items / (num_vars * 4) seems reasonable
    ...
    for P_class in targL:
        for P_set in Ps_GenL[str(P_class)]:
            ...
            if targ_mon2 != "none":
                ...
                if not temp2:
                    continue  # Should verify if this matches original logic, original logic stops if temp2 empty inside inner if. Yes.

            # Chunk temp1
            # Dynamic chunk size: large enough to be worth processing, small enough to distribute
            # Heuristic: at least 10 items per chunk, split into at most num_vars * 4 chunks
            n_items = len(temp1)
            n_chunks = num_vars * 4
            chunk_size = max(10, (n_items + n_chunks - 1) // n_chunks)

            for i in range(0, n_items, chunk_size):
                chunk = temp1[i : i + chunk_size]
                # args tuple
                task_args = (
                    str(P_class),
                    int(Ps_rxnL_key),
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        results = list(executor.map(_process_polymer_generation_chunk, tasks))

    for res in results:
        if not res.empty:
            DF_Pgen = pd.concat([DF_Pgen, res], ignore_index=True, copy=False)
    ...
    # Check if empty, calling values on empty df can raise error in some pandas versions or just be safe
    if not DF_gendP.empty:
        DF_gendP["reactset"] = np.sort(
            DF_gendP.loc[:, ["mon1", "mon2"]].values
        ).tolist()
        DF_gendP["reactset"] = DF_gendP["reactset"].apply(set).apply(tuple)
        DF_gendP = DF_gendP.drop_duplicates(subset=["reactset", "polym"])
    else:
        # Create empty reactset column to match schema if needed or just return empty
        DF_gendP["reactset"] = []
