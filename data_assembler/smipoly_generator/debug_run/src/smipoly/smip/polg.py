...
def biplym(df, targ=None, dsp_rsl=None):
    ...
    for P_class in targL:
        for P_set in Ps_GenL[str(P_class)]:
            ...
            # print(f"[++] Started executing for monomers {P_set[0]} + {P_set[1]}", flush = True)
            DF10 = DF[DF[targ_mon1]]
            temp1 = list(DF10['smip_cand_mons'])
            if len(temp1) != 0:
                if targ_mon2 != 'none':
                    if targ_mon1 not in relevant_mon_classes and targ_mon2 not in relevant_mon_classes:
                        print(f"[ -- ] The monomer {targ_mon1} not in the relevant monomer sets")
                        continue
                    ...
                    if targ_mon1 not in relevant_mon_classes:
                        print(f"[ -- ] The monomer pair of {targ_mon1} and {targ_mon2} not in the relevant monomer sets")
                        continue
...
def biplym_mt(df, targ=None, dsp_rsl=None, max_workers=None):
    ...
    for P_class in targL:
        for P_set in Ps_GenL[str(P_class)]:
            ...
            if not (targ_mon1 in relevant_mon_classes or targ_mon2 in relevant_mon_classes):
                continue
