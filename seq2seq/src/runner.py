...
5. Benchmarking sampling outputs with various metrics
...
Usage:
    ...
    # Benchmarking
    # Benchmark using an experiment directory (finds samples automatically)
    python runner.py benchmark --experiment experiments/gru_20240806_123456

    # Benchmark using a specific input file
    python runner.py benchmark --input-file data/samples.csv --output-dir results/benchmarks

    # Benchmark with custom configuration
    python runner.py benchmark --config config/benchmark/custom_config.yaml
...
class ConfigManager:
    ...
    def __init__(self, config_root: str = "config"):
        ...
        self.benchmark_config_dir = os.path.join(config_root, "benchmark")
    ...
    def load_trainer_config(
        self, model_type: str, custom_config: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        model_config_path = os.path.join(
            self.trainer_config_dir, f"{model_type}_config.yaml"
        )
    ...
    def load_sampler_config(
        ...
        # Load model-specific configuration
        model_config_path = os.path.join(
            self.sampler_config_dir, f"{model_type}_config.yaml"
        )
        model_config = self.load_yaml(model_config_path)
    ...
    def load_benchmark_config(
        self, custom_config: Optional[str] = None
    ) -> Dict[str, Any]:
        """Load benchmark configuration."""
        # Load base configuration
        base_config_path = os.path.join(self.benchmark_config_dir, "base_config.yaml")
        base_config = self.load_yaml(base_config_path)
...
class ModelRunner:
    ...
    def sample_from_model(self, args):
        ...
        if args.experiment:
            ...
            if not model_type:
                logger.error(
                    "Model type must be specified when using direct checkpoint path"
                )
                return False
    ...
    def visualize_structure(self, args) -> bool:
        ...
        canvas_width = (
            args.canvas_width
            if getattr(args, "canvas_width", None) is not None
            else canvas_cfg.get("width", 1400)
        )
        canvas_height = (
            args.canvas_height
            if getattr(args, "canvas_height", None) is not None
            else canvas_cfg.get("height", 500)
        )
        fig_cfg = canvas_cfg
        figure_width = (
            args.figure_width
            if getattr(args, "figure_width", None) is not None
            else fig_cfg.get("figure_width", 10)
        )
        figure_height = (
            args.figure_height
            if getattr(args, "figure_height", None) is not None
            else fig_cfg.get("figure_height", 6)
        )
        output_dir = (
            args.output_dir
            if getattr(args, "output_dir", None)
            else config.get("output", {}).get("directory", "./visualizations")
        )

        # Configure LogP coloring
        logp_config = config.get("logp", {})
        logp_colors = logp_config.get(
            "colors", {"philic": [0.0, 1.0, 1.0, 1], "phobic": [1.0, 1.0, 0.0, 1]}
        )
        ...
        visualizer = CustomVisualizer(
            ...
            logP_color=None,
        )  # We'll apply LogP coloring through the filter
            ...
            if getattr(args, "batch", False):
                ...
                if ext == ".csv":
                    try:
                        ...
                        return False
                    csv_col = getattr(args, "csv_column", None) or config.get(
                        "batch", {}
                    ).get("csv_column", "SMILES")
                    logp_columns = config.get("logp", {}).get("columns", {})
                    logp1_col = getattr(args, "logp1_column", None) or logp_columns.get(
                        "logp1", "LogP1"
                    )
                    logp2_col = getattr(args, "logp2_column", None) or logp_columns.get(
                        "logp2", "LogP2"
                    )
                    ...
                    if logp1_col and logp2_col:
                        source = (
                            "command line"
                            if getattr(args, "logp1_column", None)
                            else "config"
                        )
                    ...
                    for i, chunk in enumerate(
                        ...
                        if logp1_col and logp2_col:
                            if (
                                logp1_col not in chunk.columns
                                or logp2_col not in chunk.columns
                            ):
                                logger.error(
                                    f"LogP columns not found in CSV: expected '{logp1_col}' and '{logp2_col}'"
                                )
                                return False
                            ...
                            try:
                                ...
                                logger.info(
                                    f"Using per-molecule LogP values from columns {logp1_col} and {logp2_col}"
                                )
                        ...
                        visualizer.save_image_batch(
                            data_list,
                            output_file_intro=f"polymer_batch_{i + 1}",
                    ...
                    logp_filters = (
                        [logP_filter] * len(data_list) if logP_filter else None
                    )
                    visualizer.save_image_batch(
                        data_list, output_file_intro="polymer", logP_filter=logp_filters
                    )
            else:
                ...
                visualizer.save_image(
                    data,
                    filename=f"{config.get('output', {}).get('file_prefix', 'polymer')}.png",
    ...
    def benchmark_outputs(self, args: argparse.Namespace) -> bool:
        """Run benchmarking on sampling outputs."""
        logger.info("Starting benchmarking...")

        # Load config
        config_manager = ConfigManager()
        config = config_manager.load_benchmark_config(args.config)

        # Override config with args
        if args.experiment:
            config["experiment_dir"] = args.experiment
        if args.input_file:
            config["input_file"] = args.input_file
        if args.output_dir:
            config["output_dir"] = args.output_dir
        if args.csv_column:
            config["csv_column"] = args.csv_column

        try:
            from src.benchmarking.run_bench import run_benchmark

            return run_benchmark(config)
        except ImportError as e:
            logger.error(f"Failed to import benchmarking module: {e}")
            return False
        except Exception as e:
    ...
    def _build_trainer_kwargs(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if logging_config.get("use_wandb", False):
            ...
            kwargs["project_name"] = logging_config.get(
                "project_name", "polymer-training"
            )
    ...
    def _build_sampler_kwargs(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            "experiment_dir" in experiment_config
            and experiment_config["experiment_dir"]
        ):
    ...
    def _build_filter_kwargs(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "enabled" in filtering_config.get("discriminator", {}):
            if not filtering_config["discriminator"]["enabled"]:
                return kwargs
            kwargs["discriminator_enabled"] = filtering_config["discriminator"][
                "enabled"
            ]
        if "model_type" in filtering_config.get("discriminator", {}):
            kwargs["discriminator_model_type"] = filtering_config["discriminator"][
                "model_type"
            ]
        if "model_dir" in filtering_config.get("discriminator", {}):
            ...
            if os.path.exists(filtering_config["discriminator"]["model_dir"]) is False:
                raise ValueError(
                    f"discriminator.model_dir path does not exist: {filtering_config['discriminator']['model_dir']}"
                )
            kwargs["discriminator_model_dir"] = filtering_config["discriminator"][
                "model_dir"
            ]
        if "logp_threshold" in filtering_config.get("discriminator", {}):
            kwargs["discriminator_logp_threshold"] = filtering_config["discriminator"][
                "logp_threshold"
            ]
...
def create_argument_parser():
    ...
    train_parser.add_argument(
        "--device", choices=["auto", "cuda", "cpu"], help="Device to use"
    )
    train_parser.add_argument(
    ...
    sample_parser.add_argument(
        "--model", help="Model type (required when using --checkpoint)"
    )
    ...
    sample_parser.add_argument(
        "--num-sequences", type=int, help="Number of sequences to generate"
    )
    ...
    config_parser = subparsers.add_parser(
        "show-config", help="Show configuration for a model"
    )
    ...
    config_parser.add_argument(
        "--mode", required=True, choices=["train", "sample"], help="Configuration mode"
    )
    ...
    visual_parser.add_argument(
        "--config", help="Custom visualization configuration file"
    )
    visual_parser.add_argument(
        "--output-dir", help="Output directory for visualizations"
    )
    visual_parser.add_argument(
        "--canvas-width", type=int, help="Width of the canvas for visualization"
    )
    visual_parser.add_argument(
        "--canvas-height", type=int, help="Height of the canvas for visualization"
    )
    visual_parser.add_argument(
        "--figure-width", type=float, help="Width of the figure in inches"
    )
    visual_parser.add_argument(
        "--figure-height", type=float, help="Height of the figure in inches"
    )
    visual_parser.add_argument(
        "--logp-filter", action="store_true", help="Enable logP filtering"
    )
    visual_parser.add_argument(
        "--logp1", type=float, help="LogP value for first fragment"
    )
    visual_parser.add_argument(
        "--logp2", type=float, help="LogP value for second fragment"
    )
    visual_parser.add_argument(
        "--batch",
        action="store_true",
        help="Process input as batch (file with multiple SMILES strings)",
    )
    visual_parser.add_argument(
        "--csv-column",
        help="Column name containing SMILES strings in CSV file (for batch processing)",
    )
    visual_parser.add_argument(
        "--logp1-column",
        help="Column name containing LogP values for first fragment in CSV file",
    )
    visual_parser.add_argument(
        "--logp2-column",
        help="Column name containing LogP values for second fragment in CSV file",
    )

    # Benchmark command
    bench_parser = subparsers.add_parser("benchmark", help="Benchmark sampling outputs")
    bench_parser.add_argument("--config", help="Benchmarking configuration file")
    bench_parser.add_argument(
        "--experiment", help="Path to experiment directory containing samples"
    )
    bench_parser.add_argument(
        "--input-file",
        help="Path to specific input CSV file (overrides experiment directory)",
    )
    bench_parser.add_argument(
        "--output-dir", help="Output directory for benchmark results"
    )
    bench_parser.add_argument("--csv-column", help="Column name containing sequences")
...
def main():
    ...
    if args.command == "train":
        ...
        success = runner.benchmark_outputs(args)
    else:
