...
from src.sampler.config import DEFAULT_SAMPLING_CONFIG, STARTER_TEXTS, MODEL_SAMPLING_CONFIGS
...
class Sampler:
    ...
    def _resolve_checkpoint_path(self, path: str) -> str:
        ...
        if os.path.isfile(path):
            ...
            potential_paths = [
                ...
                os.path.join("experiments", path, "checkpoints"),  # Direct to checkpoints dir
    ...
    def _find_best_checkpoint_in_directory(self, directory: str) -> str:
        ...
        if os.path.isdir(epoch_checkpoints_dir):
            ...
            if pth_files:
                # Use the newest file by modification time
                newest_checkpoint = max(pth_files, key=os.path.getmtime)
                print(f"Using newest epoch checkpoint: {os.path.basename(newest_checkpoint)}")
                return newest_checkpoint
    ...
    def _load_experiment_info(self) -> Optional[Dict[str, Any]]:
        ...
        history_path = os.path.join(self.experiment_dir, "results", "training_history.json")
    ...
    def _prepare_initial_sequence(self, starter_text: Optional[str] = None) -> torch.Tensor:
        """
    ...
    def _standardize_model_output(self, outputs: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        **generation_kwargs
        ...
        with torch.no_grad():
            for step in range(generation_config['max_length']):
                ...
                # Check for EOS token
                if (generation_config.get('early_stopping', True) and 
                    next_token.item() == generation_config['eos_token_id']):
                    break
                
                # Check minimum length
                if (step + 1 >= generation_config.get('min_length', 0) and 
                    next_token.item() == generation_config['eos_token_id']):
                    break
    ...
    def generate_filter_batch(self,
                              logp_filter: Any,
                              starter_text: Optional[str] = None,
                              num_sequences: int = 5,
                              max_length: Optional[int] = None,
        ...
        **generation_kwargs
        ...
        if "discriminator_enabled" in self.filtering_config:
            ...
            logp_filter = LogPFilter(
                model_type=self.filtering_config.get('discriminator_model_type', 'dmpnn'),
                model_dir=self.filtering_config.get('discriminator_model_dir', "./"),
                logp_threshold=self.filtering_config.get('discriminator_logp_threshold', 1.0)
            )
            for result in tqdm(self.generate_filter_batch(
    ...
    def generate_with_preset(
        ...
        **generation_kwargs
        ...
        if preset_name not in STARTER_TEXTS:
            raise ValueError(f"Unknown preset: {preset_name}. Available: {list(STARTER_TEXTS.keys())}")
    ...
    def save_results(
        ...
        if use_experiment_dir and self.experiment_dir and os.path.isdir(self.experiment_dir):
    ...
    def evaluate_sequences(self, results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        evaluation = {
            ...
            'diversity_ratio': len(set(r['decoded_text'] for r in results)) / len(results)
        }
...
if __name__ == "__main__":
    ...
    experiment_path = "experiments/tcn_polymer_experiment_v1"  # Example experiment directory
    ...
    try:
        ...
        if sampler.experiment_dir:
            ...
            json_path = sampler.save_results(all_results, "generated_molecules.json", format='json')
            csv_path = sampler.save_results(all_results, "generated_molecules.csv", format='csv')
            txt_path = sampler.save_results(all_results, "generated_molecules.txt", format='txt')
            ...
            print(f"   📁 Samples: {os.path.join(sampler.experiment_dir, 'results', 'samples')}")
        else:
            print(f"⚠️  Warning: No experiment directory found in checkpoint. This shouldn't happen with the new structure.")
    
    except FileNotFoundError as e:
        print(f"❌ Error: {e}")
        print(f"\n💡 Usage examples:")
        print(f"   📁 Experiment directory: 'experiments/tcn_polymer_experiment_v1'")
        print(f"   � Direct experiment path: 'tcn_polymer_experiment_v1' (searches in experiments/)")
